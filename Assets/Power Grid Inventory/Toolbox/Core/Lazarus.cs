...
namespace Toolbox
{
    ...
    public class Lazarus : GlobalSingletonMonoBehaviour<Lazarus>
    {
        ...
        /// </summary>
        public static ushort CurrentPoolSize
        {
            get
            {
                ushort count = 0;
                var list = SubPools;
                for (int i = 0; i < list.Count; i++)
                    count += (ushort)list[i].transform.childCount;
                return count;
            }
        }
        ...
        public static int MaxPossibleObjects
        {
            get
            {
                ...
                var list = SubPools;
                for (int i = 0; i < list.Count; i++)
                    count += (ushort)list[i].MaxPoolSize;
                return count;
            }
        }
        ...
        protected override void AutoSingletonInit() { }
        ...
        public static GameObject InstantiateFromPool(GameObject blueprint)
        {
            return Summon(blueprint, blueprint.transform.position);
        }
        ...
        /// <exception cref="ArgumentNullException" />
        [Obsolete("Use Summon instead.")]
        public static GameObject InstantiateFromPool(GameObject blueprint, Vector3 position)
        {
            return Summon(blueprint, position);
        }
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentNullException" />
        public static GameObject Summon(GameObject blueprint, bool forceCreate = false)
        {
            return Summon(blueprint, blueprint.transform.position, forceCreate);
        }
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentNullException" />
        public static GameObject Summon(GameObject blueprint, Vector3 position, bool forceCreate = false)
        {
            ...
            #endif
            if(forceCreate) return GameObject.Instantiate<GameObject>(blueprint);
            ...
            else
            {
                ...
                obj = pool.ObtainObjectNonValidate(null, Lazarus.AllocDelay);
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentNullException" />
        public static GameObject RecycleFromPool(GameObject blueprint, Vector3 position)
        {
            if (blueprint == null) throw new ArgumentNullException("blueprint");
        ...
        /// <param name="blueprint"></param>
        /// <returns></returns>
        public static void PreallocateAsync(GameObject blueprint, ushort preallocCount, ushort chunkSize, ushort maxPoolSize, float delayTime = 0)
        {
            if (blueprint == null) throw new ArgumentNullException("blueprint");
        ...
        public static void RelenquishToPool(GameObject gameObject)
        {
            ...
            int id = PoolIdValue(gameObject);

            TransformPool pool = null;
            if (Pools.TryGetValue(id, out pool))
                pool.RelenquishObject(gameObject.transform);
        ...
        /// </summary>
        public static TransformPool GetAssociatedPool(GameObject gameObject, ushort chunkSize, ushort maxPoolSize)
        {
        ...
        /// </summary>
        /// <param name="level">The number of elements that should be level in the pools after draining.</param>
        ...
        public static void RelenquishPool(TransformPool pool)
        {
            ...
            //if there is no instance, there can't possibly be any pools
            if (pool != null && Instance != null) pool.RecallAll();
        }
